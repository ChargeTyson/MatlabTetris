function check = checkCanRotate(display, piece, blockNum, position)

%the process described below for the line block is the same process for
%every other block, the only difference is some blocks need to check 4
%positions for collisions while others only need to check 2.
...
if(blockNum == 1)
    ...
    if(position == 1)
        ...
        val2 = sum(piece(1:20, 10) == 11) + sum(piece(1:20, 9) == 11);
    ...
    %Same as before but checking from position 2 to 1
        ...
        val1 = sum(piece(20, 1:10) == 11) + sum(piece(19, 1:10) == 11);
        val2 = sum(piece(1, 1:10) == 11);
        if(val1 > 0 || val2 > 0)
            check = false;
        else
            found = true;
